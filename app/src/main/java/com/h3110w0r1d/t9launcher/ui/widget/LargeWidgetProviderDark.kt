...
/**
 * 大尺寸深色背景Widget的Provider类
 * 负责创建、更新Widget并处理点击事件
 */
class LargeWidgetProviderDark : AppWidgetProvider() {
    ...
    private fun updateAppWidget(
        ...
        val pendingIntent = PendingIntent.getActivity(
            context,
            345, // 使用不同的请求码避免与其他Widget冲突
            intent,
            PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE
        )

        // 获取Widget的远程视图，使用新的深色背景布局
        val views = RemoteViews(context.packageName, R.layout.widget_large_dark)
        ...
        // 获取保存的图标位置设置
        val position = IconPositionActivity.getCurrentIconPosition(context, true)
    ...
    override fun onEnabled(context: Context) {
        ...
        val componentName = ComponentName(context, LargeWidgetProviderDark::class.java)
